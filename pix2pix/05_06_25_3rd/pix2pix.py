...
def weighted_l1_loss(pred, target, device):
    """Pérdida L1 ponderada por canal Stokes"""
    # Dar más peso al canal I (intensidad)
    weights = torch.tensor([3.0, 1.0, 1.0, 1.0]).to(device)
    
    channel_losses = []
    for i in range(4):
        loss_i = F.l1_loss(pred[:, i], target[:, i])
        channel_losses.append(loss_i * weights[i])
    
    return sum(channel_losses) / sum(weights)
    ...
    for epoch in range(CONFIG['num_epochs']):
        ...
        for batch_idx, (noisy_batch, clean_batch) in enumerate(pbar):
            try:
                ...
                g_l1_loss = weighted_l1_loss(fake_clean, clean_batch, device)
